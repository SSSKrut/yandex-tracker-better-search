...
- The UI runs under a strict CSP (`script-src 'self'`, no `unsafe-inline`): templates must contain no `<script>`/`<style>` blocks, no `style=` attributes and no `on*=` handlers — put CSS/JS in `internal/server/static/` (embedded, served at `/static/`) and attach handlers with `addEventListener`. `TestTemplatesHaveNoInlineCode` fails the build otherwise. htmx is vendored locally, so the UI has no CDN dependency at runtime.
- Search highlighting is not HTML: `HIGHLIGHT()` returns control-character markers (`indexer.HighlightOpen`/`HighlightClose`) mixed with issue text, and `highlightHTML` escapes the whole string before turning the markers into `<b>`. Never render `.Highlight` raw — Manticore's `html_strip` decodes entities and can hand back live markup.
